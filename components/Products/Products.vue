<template>
  <div class="products">
    ...
    <div class="setting">
      <div class="box-total">
        <p class="total">
          Showing {{ StartLimit + 1 }} -
          {{ items.length < EndLimit + 1 ? items.length : EndLimit + 1 }} of
      ...
      <div class="box-filter">
        ...
        <div class="box-sort">
          ...
          <div class="items" v-if="Dropdown">
            <div class="item" id="rendah" @click="ChangeSort('Asc')">
              Harga Terendah
            </div>
            <div class="item" id="tinggi" @click="ChangeSort('Desc')">
              Harga Tertinggi
            </div>
          </div>
        </div>
        <div class="box-search">
          ...
          <input
            class="input"
            type="text"
            placeholder="Cari di ProjectByAmi"
    ...
    <div class="content">
      <BoxProduct
        ...
        v-show="index >= StartLimit && index <= EndLimit"
        :counter="index + 1"
        :nama="item.nama"
        :img="item.img"
        :harga="item.harga" />
    ...
    <div class="numberPage">
      <button class="button" @click="MoveFirstPage()">
        <img class="logo" src="assets/products/Prev2.svg" alt="" />
      </button>
      <button class="button" @click="PrevPage()">
        <img class="logo" src="assets/products/Prev1.svg" alt="" />
      </button>
      <button
        class="button"
        ...
        v-show="n >= PageShowStart && n <= PageShowEnd">
        {{ n }}
      </button>
      <button
        class="button"
        v-show="PageShowEnd + 1 < Math.ceil(items.length / 12)"
        disabled>
        ...
      </button>
      <button
        class="button"
        v-show="Math.ceil(items.length / 12) > 3"
        @click="SelectPage(Math.ceil(items.length / 12))"
        :class="{ current: CurrentPage == Math.ceil(items.length / 12) }">
        {{ Math.ceil(items.length / 12) }}
      </button>
      <button class="button" @click="NextPage()">
        <img class="logo" src="assets/products/Next1.svg" alt="" />
      </button>
      <button class="button" @click="MoveLastPage()">
        <img class="logo" src="assets/products/Next2.svg" alt="" />
      </button>
    </div>
  </div>
</template>
...
<script>
import { ref } from "vue";
import image from "@/assets/products/Product_1.svg";
  ...
  data() {
    ...
    function PrevPage() {
      if (
        ...
        (CurrentPage.value + 1 == Math.ceil(this.items.length / 12) ||
          CurrentPage.value == PageShowStart.value)
      ) {
        PageShowStart.value = PageShowStart.value - 1;
        PageShowEnd.value = PageShowEnd.value - 1;
      }
    ...
    function SelectPage(SelectedPage) {
      ...
      if (SelectedPage == 1) {
        ...
        if (
          SelectedPage == Math.ceil(this.items.length / 12) ||
          SelectedPage == Math.ceil(this.items.length / 12) - 1
        ) {
          PageShowStart.value = Math.ceil(this.items.length / 12) - 3;
          PageShowEnd.value = Math.ceil(this.items.length / 12) - 1;
    ...
    const SearchItems = (props) => {
      ...
      if (props.length == 0) {
        ...
        const filterItems = SemuaItem.filter(
          (p) => p.nama.toLowerCase().indexOf(props.toLowerCase()) >= 0
        );
